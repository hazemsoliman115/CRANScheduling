...
if __name__ == "__main__":
    ...
    #Num_Flows_List = [5]
    NumRes = 128
    clf_list = [svm.SVC(kernel='rbf', class_weight='auto', gamma=0, C=1), DecisionTreeClassifier(max_depth=5, class_weight={1: 70})]
    ...
    for clf_index in range(len(clf_list)):
        for nflow_ind in range(len(Num_Flows_List)):
            ...
            for TTI in range(600):#range(len(List_Flows[0].channel_profile[0])):
                ...
                select_flow = None
                for y in List_Flows:
                    if y.channel_profile[0][TTI] >= max_weight:
            ...
            for y in List_Flows:
                for i in range(600):#range(len(y.channel_profile[0])):
                    X.append([y.channel_profile[0][i]])
...
#            plt.plot(YX,clf.predict(X),'*')
            ...
            for TTI in range(600):#range(len(List_Flows[0].channel_profile[0])):
                ...
                select_flow = None
                for y in List_Flows:
                    if y.channel_profile[40][TTI] >= max_weight:
            ...
            for y in List_Flows:
                for i in range(600):#range(len(y.channel_profile[0])):
                    X_T.append([y.channel_profile[40][i]])
